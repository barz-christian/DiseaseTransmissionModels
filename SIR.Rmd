...
In this note we model the outbreak of a disease using a compartment model, more precisely a Susceptible-Infected-Recovered (SIR) model. We show how in `Stan` the corresponding systems of differential equations can be solved an how to find optimal optimal model parameters with a bayesian approach.

Moreover we compute useful epidemilogical parameters as the *basic reproduction rate* $R_0$.
...
In our example we are examine an outbreak of influenza A (H1N1) in 1978 at a british school

[Anonymous. 1978. Influenza in a boarding school. British Medical Journal 1:578.](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1603269/pdf/brmedj00115-0064.pdf). The data is freely available in the R package `outbreaks` and consists of the daily number of students in bed spanning over a time interval of 14 days.
...
We will model the outbreak by a SIR-model, which we explain in the next section.

## Deterministic model {#SIR_model}

A susceptible-infected-recoved model (SIR) splits the population in three time-dependent compartments:
...
- $R(t)$ is the number of recovered people (we assume they remain immune),
- $N$ is the total number of the population,
- $\beta$ is the constant rate of infection,
...
Let $\beta$ denotes the average number of contacts per person per time.
Then at each time step the probability for a susceptible person to become infected is $\beta\frac{I}{N}$. 
...
Let $\gamma$ denote the recovery rate. Hence the number of infected people decreases with a speed of $\gamma I$ and is equal to the grows of the compartment $R$.
...
- replacing the integer number of people in each compartment by a continuous approximation is legitimate (in other words the population is big enough)


## Probability model

The differential system from the last section can be solved numerically by using for example a Runge-Kutta method. But beside the initial conditions $S(0),I(0),R(0)$ we have to chose values for the parameters $\beta$ and $\gamma$. This is the part were bayesian statistics is joining the party. So let us recall some basics from bayesian statistics:

### Bayes rule

Let $A$ and $B$ be two events. The conditional probability of $A$ given $B$, denoted by $P(A|B)$, is the likelihood of observing event $A$ given that $B$ is true and **Bayes rule** is the following formula:

$$
P(A|B)=\frac{P(B|A)P(A)}{P(B)}.
$$

To understand how Bayes rule will help us, we slightly change our notation: We denote by $D$ our observed data and by $\mathcal{H}$ our model, which is determined by its parameters $\beta$ and $\gamma$.

Then the conditional probability $P(\mathcal{H},D)$, measures the likelihood of the model $\mathcal{H}$ given the data $D$?

This question is equal to $P(\mathcal{H}|D)$  and Bayes rule tells us how to compute it, namely:
...
- The **sampling distribution** $P(D|\mathcal{H})$ measures how likely it is to observe the data $D$ given the hypothesis $\mathcal{H}$.
...
Assume the parameters and initial conditions are given, then the compartment model defines a unique solution for each compartment, including the number of infected people, $I_{ODE}(t)$. At each time point We want to link this solution to the observed data, i.e. number of students in bed $I_{obs}(t)$. As the latter is a noisy estimate of the true number of infected people, we model this number $I_{obs}(t)$ with a count distribution, precisely the negative binomial distribution. This distribution allows us to use  $I_{ODE}(t)$ as the expected value and account for over-dispersion by the parameter $\phi$:
...
Next we specify a prior distribution $P(\mathcal{H})$ with $\mathcal{H}=(\beta,\gamma,\phi)$ for each of the parameters. This is an advantage of the Bayesian approach, as this way we can include prior knowledge about the paratermeters in the model.
...
This expresses our beliefs, that $\gamma$ has to be positive and the probability that the recovery time is bigger than a day is a priori 90%. Of course we can change this prior according to expert knowledge and if more information become available.
...
In this section show how a system of ordinary differential equations, e.g. a 
susceptible-infected-recovered model (SIR), can be solved in `Stan`.
...
where $y$ are the *states*, more precisely the components of $y$ are the 
compartments in our example, $y = (S,I,R)$, and $t$ is the time. We denote the 
initial conditions by $y_0$ and $t_0$ and by $\tau$ the times at which we 
evaluate the solution.

In order to specify an ODE in Stan, we first code $f$ in the `function block`. 
...
```{r traceplot}
traceplot(fit_sir_negbin, pars = pars, inc_warmup = TRUE)
```
